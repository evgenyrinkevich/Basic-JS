...
function hideMoreText(card) {
    ...
    card.productName.removeChild(card.wrap.querySelector('.desc'));
...
function showMoreText(card) {
    ...
    const randomText = 'This is an awesome product! Buy immediately';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    const divDesc = document.createElement('div');
    divDesc.innerText = randomText;
    divDesc.style.color = 'lightgrey';
    divDesc.classList.add('desc');
    card.productName.appendChild(divDesc);
